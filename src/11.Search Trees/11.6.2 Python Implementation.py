...
class RedBlackTreeMap(TreeMap):
    ...
    class _Node(TreeMap._Node):
        ...
        __slots__ = '_red'      # add additional data menber to the Node class
        
        def __init__(self,element,parent=None,left=None,right=None):
            super().__init__(element,parent,left,right)
            self._red = True    # new node red by default
    #------------------------- positional-based utility methods -------------------------
    # we consider a nonemistent child to be trivially black
    ...
    def _is_red_leaf(self,p): return self._si_red(p) and self.is_leaf(p)
        ...
        if self.is_root(p):
            ...
            if self._is_red(parent):    # double red problem
                ...
                if not self._is_red(uncle): # Case 1: mishapen 4-node
                    ...
                    self._set_black(middle)         # and then fix color
    ...
    def _fix_deficit(self,z,y):
        ...
        if not self._is_red(y): # y is black; will apply Case 1 or 2
            ...
            if x is not None: # Case 1: y is black and has red child x; do "trensfer"
